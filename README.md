...
**Custom** 模式用来自定义相机机位，切换到 **Custom** 模式，编辑器面板中显示 `Camera Actors` 选项，点击 【+】新增时会自动控制（Pilot）新的相机：

![image-20250125132939563](Resources/image-20250125132939563.png)

在构建好相机阵列和设置好捕获配置后，点击【Capture】按钮，将在插件的工作目录下生成数据集，点击【Browse】 按钮，可以浏览捕获结果：
...
很少情况下会出现异常情况，但为了以防万一，作者增加了 colmap 各个阶段执行的扩展参数，你可以依据以下文档来调整测试：
...
- 通常是 `{PluginDir}/WorkHome/Cache/GaussianSplattingEditor/sparse/0/`
...
如果最终导出的结果是 静态网格体，需要在放入场景后，勾选 静态网格体组件的 **三角形排序（Sort Triangles）** 

> 由于虚幻引擎的 **Order Independent Translucency** 算法最多只支持 `256` 个排序层级，因此大量重叠的高斯点可能会出现闪烁现象。

![image-20250125145736912](Resources/image-20250125145736912.png)
